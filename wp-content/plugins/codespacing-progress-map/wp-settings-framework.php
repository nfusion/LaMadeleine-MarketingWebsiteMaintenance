    ...
    class CsPm_WordPressSettingsFramework {
    	...
    	function cspm_admin_enqueue_scripts()
    	{
			...
			if (is_admin()) {
				...
				$use_ssl = cspm_wpsf_get_setting( cspm_wpsf_get_option_group( $this->plugin_path .'settings/codespacing-progress-map.php' ), 'troubleshooting', 'use_ssl' );				
				if(!$use_ssl) $use_ssl = 'http';
				
				wp_register_script('cspacing_pm_livequery_js', $this->plugin_url .'js/min/jquery.livequery.min.js', array( 'jquery' ));
				 wp_enqueue_script('cspacing_pm_livequery_js');
				
				wp_register_script('codespacing_pm_maps_google', ''.$use_ssl.'://maps.google.com/maps/api/js?sensor=true', array( 'jquery' ));
				 wp_enqueue_script('codespacing_pm_maps_google');
				 
				wp_register_script('codespacing_pm_gmap_js', $this->plugin_url .'js/min/gmaps.min.js', array( 'jquery' ));
				...
				if (isset($_GET['page']) && $_GET['page'] == $this->plugin_get_var ){
					...
					wp_register_style('cspacing_pm_admin_css', $this->plugin_url .'css/admin_style.css');
					 wp_enqueue_style('cspacing_pm_admin_css');
					
					wp_register_style('cspacing_pm_icheck_css', $this->plugin_url .'css/icheck/all.css?v=0.9.1');
					 wp_enqueue_style('cspacing_pm_icheck_css');
					
					wp_register_style('cspacing_pm_multi_select_css', $this->plugin_url .'css/multi-select.css');
					...
					wp_register_script('cspacing_pm_jquery_cookie', $this->plugin_url .'js/jquery_cookie.js', array( 'jquery' ));
					 wp_enqueue_script('cspacing_pm_jquery_cookie');
		
					wp_register_script('cspacing_pm_jquery_validate', $this->plugin_url .'js/min/jquery.validate.min.js', array( 'jquery' ));
					 wp_enqueue_script('cspacing_pm_jquery_validate');
					 
					wp_register_script('cspacing_pm_qtip_js', $this->plugin_url .'js/min/jquery.qtip-1.0.0-rc3.min.js', array( 'jquery' ));
					 wp_enqueue_script('cspacing_pm_qtip_js');
					
					wp_register_script('cspacing_pm_cufon_font', $this->plugin_url.'js/cufon/cufon-yui.js');
					 wp_enqueue_script('cspacing_pm_cufon_font');
					 
					wp_register_script('cspacing_pm_cufon_Linux_Biolinum', $this->plugin_url.'js/cufon/Linux_Biolinum_400.font.js');
					 wp_enqueue_script('cspacing_pm_cufon_Linux_Biolinum');
					
					wp_register_script('cspacing_pm_multi_select_js', $this->plugin_url .'js/jquery.multi-select.js', array( 'jquery' ));
 					 wp_enqueue_script('cspacing_pm_multi_select_js');
					 
					wp_register_script('cspacing_pm_quicksearch_js', $this->plugin_url .'js/jquery.quicksearch.js', array( 'jquery' ));
 					 wp_enqueue_script('cspacing_pm_quicksearch_js');
					 
					wp_register_script('cspacing_pm_admin_script', $this->plugin_url .'js/admin_script.js', array( 'jquery' ));
        	...
        	if(!empty($wpsf_settings)){
        		...
        		foreach($wpsf_settings as $section){
            		if(isset($section['section_id']) && $section['section_id'] && isset($section['section_title'])){                		
						add_settings_section( $section['section_id'], $section['section_title'], array(&$this, 'cspm_section_intro'), $this->option_group );
                		if(isset($section['fields']) && is_array($section['fields']) && !empty($section['fields'])){
                    		foreach($section['fields'] as $field){
                        		if(isset($field['id']) && $field['id'] && isset($field['title'])){									
                        		    add_settings_field( $field['id'], '<strong>'.$field['title'].'</strong>', array(&$this, 'cspm_generate_setting'), $this->option_group, $section['section_id'], array('section' => $section, 'field' => $field) );
										...
										if(count($taxonomy_terms) > 0){
											foreach($taxonomy_terms as $single_taxonomy){
												foreach($single_taxonomy as $taxonomy_data){												
													add_settings_field($taxonomy_data['id'], '<strong>'.$taxonomy_data['title'].'</strong>', array(&$this, 'cspm_generate_setting'), $this->option_group, $section['section_id'], array('section' => $section, 'field' => $taxonomy_data) );
												}
											}
										}
									}
									...
									if($field['id'] == "marker_cats_settings"){
										...
										if(count($taxonomies_data) > 0)
											add_settings_field($taxonomies_data['id'], '<strong>'.$taxonomies_data['title'].'</strong>', array(&$this, 'cspm_generate_setting'), $this->option_group, $section['section_id'], array('section' => $section, 'field' => $taxonomies_data) );
									}
									// End
        ...
        function cspm_settings()
        {
            ...
            <form action="options.php" method="post" id="cspm_form">
				...
				<?php cspm_custom_do_settings_sections( $this->option_group ); ?>
